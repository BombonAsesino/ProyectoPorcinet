...
import CostsChartsScreen from "./CostsChartsScreen";
import FarmLocationScreen from "./FarmLocationScreen";
// ⬇️ Inventario de Insumos
import InventarioInsumosScreen from "./InventarioInsumosScreen";

// ⬇️ Pantalla modal de búsqueda
import SearchOverlayScreen from "./SearchOverlayScreen";

// ⬇️ Salud y Crecimiento
import HealthAndGrowthScreen from "./HealthAndGrowthScreen";

// ⬇️ Sync de colas offline de salud/crecimiento
...
function HomeMenu({ navigation }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (rol === "subcuenta") {
          offRTDB = onValue(
            ref(realtimeDb, `userPerms/${u.uid}/modules`),
            (snap) => {
              const arr = snap.val();
    ...
    const off = onValue(herdRef, async (snap) => {
      ...
      try {
        await AsyncStorage.setItem(
          STATS_KEY,
          JSON.stringify({ herdSize: total, sows: msows })
        );
      } catch {}
    });
    return () => off();
  ...
  useEffect(() => {
    ...
    const pRef = ref(
      realtimeDb,
      `producers/${u.uid}/metrics/productivityPct`
    );
    ...
    const sub = DeviceEventEmitter.addListener(
      ...
      async (payload) => {
        ...
        try {
          await AsyncStorage.setItem(
            STATS_KEY,
            JSON.stringify({ herdSize: t, sows: s })
          );
        } catch {}
      }
    );
  ...
  const saveSows = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem(
        STATS_KEY,
        JSON.stringify({ herdSize: herd, sows: newVal })
      );
      setSows(newVal);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.beige }}>
      ...
      <View
        style={[
          styles.screen,
          { backgroundColor: Colors.beige, paddingTop: 20 },
        ]}
      >
        {/* Franja superior */}
        <View style={styles.topGreen}>
          <View style={styles.rowChips}>
            ...
            <StatChip
              title="Productividad"
              value={
                productivityPct !== null && productivityPct !== undefined
                  ? `${productivityPct}%`
                  : `-- %`
              }
            />
        ...
        <View style={styles.grid}>
          ...
          {can("geolocalizacion") && (
            <BouncyTile
              onPress={() => navigation.navigate("GeolocalizacionGranja")}
            >
              <View style={styles.tileIconBox}>
                <MaterialCommunityIcons
        ...
        >
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalRow}>
              ...
              >
                <Text
                  style={[styles.modalBtnText, { color: Colors.green }]}
                >
                  Cancelar
                </Text>
              </TouchableOpacity>
  ...
  return (
    <AuthWrapper>
      ...
      >
        ...
        <Tab.Screen
          name="InventarioTab"
          component={InventarioInsumosScreen}
          options={{
            title: "Inventario",
            tabBarIcon: ({ color, size }) => (
              <MaterialCommunityIcons
...
function ReproStack() {
  return (
    <ReproStackNav.Navigator>
      ...
      />
      <ReproStackNav.Screen
        name="HealthAndGrowth"
        component={HealthAndGrowthScreen}
        options={{ title: "Salud y crecimiento" }}
      />
    </ReproStackNav.Navigator>
  );
}
  ...
  return (
    <NavigationContainer theme={theme}>
      ...
      >
        ...
        <RootStack.Screen
          name="GeolocalizacionGranja"
          component={FarmLocationScreen}
          options={{ title: "Geolocalización de granja" }}
        />
        ...
        <RootStack.Screen
          name="CostsCharts"
          component={CostsChartsScreen}
          options={{ title: "Gráficas de costos" }}
        />
        <RootStack.Screen
          name="ReproStack"
          component={ReproStack}
          options={{ headerShown: false }}
        />
